...
function DodajProdukt() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $ilosc = $_POST['ilosc'];
        $status_dostepnosci = isset($_POST['status_dostepnosci']) ? (int)$_POST['status_dostepnosci'] : 0;
        ...
        $query = "INSERT INTO produkty 
                  (tytul, opis, data_utworzenia, data_modyfikacji, cena_netto, podatek_vat, ilosc, status_dostepnosci, kategoria, gabaryt, zdjecie) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $link->prepare($query);
        if ($stmt) {
            $stmt->bind_param(
                "ssssddiissb", 
                $tytul, $opis, $data_utworzenia, $data_modyfikacji, $cena_netto, $podatek_vat, $ilosc, 
                $status_dostepnosci, $kategoria, $gabaryt, $zdjecie
            );
            $stmt->send_long_data(10, $zdjecie);
            if ($stmt->execute()) {
                echo '<p style="color: green;">Produkt został dodany pomyślnie.</p>';
            } else {
                echo '<p style="color: red;">Błąd podczas dodawania produktu: ' . $stmt->error . '</p>';
            }
        } else {
            echo '<p style="color: red;">Błąd przygotowania zapytania: ' . $link->error . '</p>';
        }
    }
